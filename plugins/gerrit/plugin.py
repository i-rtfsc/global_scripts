...
@plugin_function(
    ...
    examples=[
        ...
        "gs gerrit push -b dev -d --remote origin"
    ]
)
def push(args: List[str] | None = None) -> CommandResult:
    ...
    if ns.help:
        help_text = (
            ...
            "  -b/--branch   Target branch (default: master)\n"
    ...
    ok, msg = _ensure_git_repo()
    if not ok:
        return CommandResult(False, error=msg, exit_code=1)

    ok, msg = _check_git_identity()
    if not ok:
        return CommandResult(False, error=msg, exit_code=1)

    ok, remote_url, err = _select_remote(ns.remote)
    if not ok:
    ...
    refspec = _build_refspec(ns.branch.strip(), reviewers, ns.drafts)
