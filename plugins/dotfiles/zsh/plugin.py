...
@subplugin("zsh")
class ZshConfigSubplugin(DotfilesPlugin):
    def __init__(self):
        super().__init__()
        self.name = "zsh"
        ...
        # Zsh 配置路径映射
        self.main_config = {
            "source": self.subplugin_dir / ".zshrc",
    ...
    @plugin_function(
        name="install",
        description={"zh": "安装 Zsh 配置", "en": "Install Zsh configuration"},
        usage="gs dotfiles zsh install [--force]",
        examples=["gs dotfiles zsh install", "gs dotfiles zsh install --force"]
    )
    async def install(self, args: List[str] = None) -> CommandResult:
        ...
        try:
            ...
            return CommandResult(success=True, output=f"Zsh 配置安装成功")
        except Exception as e:
            return CommandResult(success=False, error=f"安装配置失败: {str(e)}")

    @plugin_function(
        name="uninstall",
        description={"zh": "卸载 Zsh 配置", "en": "Uninstall Zsh configuration"},
        usage="gs dotfiles zsh uninstall",
        examples=["gs dotfiles zsh uninstall"]
    )
    async def uninstall(self, args: List[str] = None) -> CommandResult:
        ...
        try:
            ...
            if not target.exists():
                return CommandResult(success=False, error="Zsh 配置未安装")

            # 备份后删除
            await self._backup_file(target, self.backup_dir, self.name)
            target.unlink()

            return CommandResult(success=True, output="Zsh 配置已卸载（已备份）")
        except Exception as e:
            return CommandResult(success=False, error=f"卸载配置失败: {str(e)}")

    @plugin_function(
        name="backup",
        description={"zh": "备份 Zsh 配置", "en": "Backup Zsh configuration"},
        usage="gs dotfiles zsh backup",
        examples=["gs dotfiles zsh backup"]
    )
    async def backup(self, args: List[str] = None) -> CommandResult:
        ...
        try:
            ...
            if not target.exists():
                return CommandResult(success=False, error="Zsh 配置未安装，无需备份")

            await self._backup_file(target, self.backup_dir, self.name)

            return CommandResult(success=True, output="Zsh 配置已备份")
        except Exception as e:
            return CommandResult(success=False, error=f"备份配置失败: {str(e)}")

    @plugin_function(
        name="restore",
        description={"zh": "恢复 Zsh 配置", "en": "Restore Zsh configuration"},
        usage="gs dotfiles zsh restore",
        examples=["gs dotfiles zsh restore"]
    )
    async def restore(self, args: List[str] = None) -> CommandResult:
        ...
        try:
            ...
            return CommandResult(success=True, output=f"Zsh 配置已恢复: {chosen_backup['name']}")
        except Exception as e:
            return CommandResult(success=False, error=f"恢复配置失败: {str(e)}")

    @plugin_function(
        name="status",
        description={"zh": "查看 Zsh 配置状态", "en": "Show Zsh configuration status"},
        usage="gs dotfiles zsh status",
        examples=["gs dotfiles zsh status"]
    )
