...
@subplugin("vim")
class VimConfigSubplugin(DotfilesPlugin):
    def __init__(self):
        super().__init__()
        self.name = "vim"
        ...
        # Vim 配置路径映射
        self.main_config = {
            "source": self.subplugin_dir / ".vimrc",
    ...
    @plugin_function(
        name="install",
        description={"zh": "安装 Vim 配置", "en": "Install Vim configuration"},
        usage="gs dotfiles vim install [--force]",
        examples=["gs dotfiles vim install", "gs dotfiles vim install --force"]
    )
    async def install(self, args: List[str] = None) -> CommandResult:
        ...
        try:
            ...
            return CommandResult(success=True, output=f"Vim 配置安装成功")
        except Exception as e:
            return CommandResult(success=False, error=f"安装配置失败: {str(e)}")

    @plugin_function(
        name="uninstall",
        description={"zh": "卸载 Vim 配置", "en": "Uninstall Vim configuration"},
        usage="gs dotfiles vim uninstall",
        examples=["gs dotfiles vim uninstall"]
    )
    async def uninstall(self, args: List[str] = None) -> CommandResult:
        ...
        try:
            ...
            if not target.exists():
                return CommandResult(success=False, error="Vim 配置未安装")

            # 备份后删除
            await self._backup_file(target, self.backup_dir, self.name)
            target.unlink()

            return CommandResult(success=True, output="Vim 配置已卸载（已备份）")
        except Exception as e:
            return CommandResult(success=False, error=f"卸载配置失败: {str(e)}")

    @plugin_function(
        name="backup",
        description={"zh": "备份 Vim 配置", "en": "Backup Vim configuration"},
        usage="gs dotfiles vim backup",
        examples=["gs dotfiles vim backup"]
    )
    async def backup(self, args: List[str] = None) -> CommandResult:
        ...
        try:
            ...
            if not target.exists():
                return CommandResult(success=False, error="Vim 配置未安装，无需备份")

            await self._backup_file(target, self.backup_dir, self.name)

            return CommandResult(success=True, output="Vim 配置已备份")
        except Exception as e:
            return CommandResult(success=False, error=f"备份配置失败: {str(e)}")

    @plugin_function(
        name="restore",
        description={"zh": "恢复 Vim 配置", "en": "Restore Vim configuration"},
        usage="gs dotfiles vim restore",
        examples=["gs dotfiles vim restore"]
    )
    async def restore(self, args: List[str] = None) -> CommandResult:
        ...
        try:
            ...
            return CommandResult(success=True, output=f"Vim 配置已恢复: {chosen_backup['name']}")
        except Exception as e:
            return CommandResult(success=False, error=f"恢复配置失败: {str(e)}")

    @plugin_function(
        name="status",
        description={"zh": "查看 Vim 配置状态", "en": "Show Vim configuration status"},
        usage="gs dotfiles vim status",
        examples=["gs dotfiles vim status"]
    )
