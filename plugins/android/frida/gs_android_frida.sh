...
# gs_android_frida -p system_server -f /home/solo/code/github/global_scripts/plugins/android/frida/hook3.js
function gs_android_frida {
    ...
    # 如果js文件不存在，则设置默认的${_GS_ROOT_PATH}/plugins/android/frida/路径
    if [ ! -f ${js_file} ]; then
        if [ -z ${_GS_ROOT_PATH} ]; then
            js_file=$(pwd)/${js_file}
        else
            js_file=${_GS_ROOT_PATH}/plugins/android/frida/${js_file}
        fi
    ...
    if [[ `adb shell ls /data/local/frida/frida-inject 2> /dev/null` ]]; then
        ...
        if [ -z ${_GS_ROOT_PATH} ]; then
            ...
            root_dir=${_GS_ROOT_PATH}/plugins/android/frida
        fi
...
function gs_android_frida_server() {
    ...
    if [[ `adb shell ls /data/local/frida/frida-server 2> /dev/null` ]]; then
        ...
        if [ -z ${_GS_ROOT_PATH} ]; then
            ...
            root_dir=${_GS_ROOT_PATH}/plugins/android/frida/frida-inject
        fi
