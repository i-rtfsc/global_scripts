...
# -*- encoding: utf-8 -*-
#
# Android Build Subplugin
# - Android build system integration with complete V2 logic
# - Supports ninja, make, full build, QSSI, vendor compilation
#
...
# description:
#   zh: 清理ninja构建缓存
#   en: Clean ninja build cache
# usage: gs android build ninja-clean
# examples:
#   - gs android build ninja-clean
gs_android_build_ninja_clean() {
...
# name: make
# description:
#   zh: 使用make编译Android模块
#   en: Build Android modules using make
# usage: gs android build make [options]
# examples:
#   - gs android build make
#   - gs android build make -t sdk_pc_x86_64-userdebug -m framework
#   - gs android build make -j 8 -c 1
...
# description:
#   zh: Android完整系统编译
#   en: Full Android system build
# usage: gs android build build [options]
# examples:
#   - gs android build build
#   - gs android build build -t sdk_pc_x86_64-userdebug
#   - gs android build build -j 8 -c 1 -b <feishu_bot_token>
gs_android_build() {
    read gs_error gs_target gs_build_thread gs_ccache gs_bot gs_module <<< $(_gs_android_build_parse_opts $*)
...
# description:
#   zh: 编译QSSI（高通特有）
#   en: Build QSSI (Qualcomm specific)
# usage: gs android build qssi [options]
# examples:
#   - gs android build qssi
#   - gs android build qssi -t qssi-userdebug
#   - gs android build qssi -j 8 -c 1
gs_android_build_qssi() {
    read gs_error gs_target gs_build_thread gs_ccache gs_bot gs_module <<< $(_gs_android_build_parse_opts $*)
...
# description:
#   zh: 编译vendor分区
#   en: Build vendor partition
# usage: gs android build vendor [options]
# examples:
#   - gs android build vendor
#   - gs android build vendor -t <target>
#   - gs android build vendor -j 8 -c 1 -b <feishu_bot_token>
gs_android_build_vendor() {
    read gs_error gs_target gs_build_thread gs_ccache gs_bot gs_module <<< $(_gs_android_build_parse_opts $*)
