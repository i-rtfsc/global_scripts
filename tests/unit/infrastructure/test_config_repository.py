...
class TestConfigRepository:
    ...
    @pytest.mark.asyncio
    async def test_load_returns_empty_when_no_config(self, config_repository):
    ...
    async def test_load_returns_config(
        self, config_repository, mock_filesystem, config_path
    ):
        """Test load returns configuration"""
        # Create test config
        test_config = {"language": "en", "logging": {"level": "DEBUG"}}
    ...
    async def test_save_writes_config(
        ...
        config = {"language": "zh", "show_examples": True}
    ...
    async def test_get_returns_value_for_nested_key(
        ...
        mock_filesystem.write_json(
            config_path, {"logging": {"level": "INFO", "file": "/var/log/gs.log"}}
        )
    ...
    @pytest.mark.asyncio
    async def test_get_returns_default_when_key_not_found(self, config_repository):
    ...
    async def test_set_updates_existing_key(
        ...
        # Initial config
        mock_filesystem.write_json(
            config_path, {"language": "zh", "show_examples": False}
        )
