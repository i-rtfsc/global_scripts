...
class ConfigManager:
    ...
    def _detect_project_root(self) -> Path:
        ...
        for parent in here.parents:
            if (parent / "config" / "gs.json").exists() or (
                parent / "src" / "gscripts"
            ).exists():
                return parent
        # 3) 回退
    ...
    def _generate_default_config(self) -> Dict[str, Any]:
                        ...
                        try:
                            ...
                            if plugin_config:
                                ...
                                if plugin_name:
                                    ...
                                    logger.warning(
                                        f"系统插件 {plugin_path} 的 plugin.json 缺少 name 字段"
                                    )
                        except Exception as e:
        ...
        logger.info(
            f"生成默认配置，检测到系统插件: {list(config['system_plugins'].keys())}"
        )
        return config
    ...
    def _load_user_config(self):
        ...
        try:
            ...
            logger.info(
                "JSON配置已加载 (合并 project + user)：系统插件=%s",
                list(merged.get("system_plugins", {}).keys()),
            )
        except Exception as e:
    ...
    def _merge_configs(
        ...
        if not isinstance(result, dict):
            logger.warning("配置合并结果不是字典类型，使用空字典")
            return {}
    ...
    def _prune_missing_plugins(self, cfg: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if plugins_dir.exists():
                for p in plugins_dir.iterdir():
                    if (
                        ...
                        and any(
                            (p / f).exists()
                            for f in ("plugin.json", "plugin.py", "plugin.sh")
                        )
                    ):
            ...
            if "system_plugins" in cfg and isinstance(cfg["system_plugins"], dict):
                removed = [
                    name
                    for name in list(cfg["system_plugins"].keys())
                    if name not in system_existing
                ]
                for name in removed:
            ...
            if "custom_plugins" in cfg and isinstance(cfg["custom_plugins"], dict):
                removed = [
                    name
                    for name in list(cfg["custom_plugins"].keys())
                    if name not in custom_existing
                ]
                for name in removed:
    ...
    def _discover_custom_plugins_recursive(
        self, custom_dir: Path, parent_path: str = ""
    ):
        ...
        for item in custom_dir.iterdir():
            ...
            if plugin_json.exists():
                ...
                try:
                    ...
                    if config:
                        ...
                        if plugin_name:
                            ...
                            logger.warning(
                                f"自定义插件 {item} 的 plugin.json 缺少 name 字段"
                            )
                except Exception as e:
                ...
                nested_plugins = self._discover_custom_plugins_recursive(
                    item, nested_path
                )
