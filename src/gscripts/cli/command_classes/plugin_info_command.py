...
class PluginInfoCommand(Command):
        ...
        if isinstance(router_commands, dict) and router_commands:
            for cmd_key, cmd_info in router_commands.items():
                ...
                if isinstance(cmd_desc_data, dict):
                    cmd_description = cmd_desc_data.get(language, cmd_desc_data.get('zh', ''))
                else:
            ...
            for func_name, func_obj in functions.items():
                ...
                if isinstance(func_obj, dict):
                    ...
                    if isinstance(func_desc, dict):
                        func_description = func_desc.get(language, func_desc.get('zh', func_desc.get('en', '')))
                    else:
    ...
    async def execute(self, args: List[str]) -> CommandResult:
        ...
        try:
            ...
            plugin_info_data = router_plugins.get(plugin_name) or loaded_plugins.get(plugin_name) or {}
