...
class GlobalScriptsCLI:
    ...
    def __init__(self, chinese: bool = None):
        ...
        logger.debug(
            f"Initializing PluginManager with plugins_root={sanitize_path(plugins_dir)}"
        )
        ...
        from gscripts.infrastructure.persistence.plugin_repository import (
            PluginRepository,
        )
    ...
    async def run(self, args: List[str] = None):
        ...
        try:
            ...
            logger.debug(
                f"Processing command: {command_name}, full_args={redact(args)}"
            )
            ...
            if result.success:
                ...
                if result.output:
                    ...
                    error_text = (
                        ...
                        or self.i18n.get_message("commands.command_failed")
                    )
    ...
    def handle_shell_function(self, function_name: str, args: List[str] = None):
        ...
        if function_name.startswith("gs_") or function_name.startswith("gs-"):
            ...
            clean_name = (
                function_name[3:]
                if function_name.startswith("gs_")
                else function_name[3:]
            )
            ...
            if clean_name in ["help", "version"]:
                ...
                logger.debug(
                    f"Resolved as plugin command: {command_args[0]} {command_args[1]}"
                )
            else:
                # 尝试解析插件命令
                ...
                if len(parts) == 2:
                    ...
                    logger.debug(
                        f"Resolved as plugin function: plugin={parts[0]}, function={parts[1]}"
                    )
                else:
