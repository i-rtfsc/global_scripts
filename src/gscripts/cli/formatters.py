...
class ChineseFormatter:
    ...
    def format_help_usage() -> str:
        ...
        h1 = i18n.get_message("cli.command")
        h1 = h1 if h1 != "cli.command" else ("命令" if lang == "zh" else "Command")
        h2 = i18n.get_message("cli.description")
        h2 = (
            h2
            if h2 != "cli.description"
            else ("描述" if lang == "zh" else "Description")
        )
        headers = [h1, h2]

        # Rows using i18n (第一列展示命令字符串，第二列展示描述)
        basic = i18n.get_message("cli.basic_info")
        basic = (
            basic
            if basic != "cli.basic_info"
            else ("基本用法" if lang == "zh" else "Basics")
        )
        usage_word = i18n.get_message("cli.usage")
        usage_word = (
            usage_word
            if usage_word != "cli.usage"
            else ("用法" if lang == "zh" else "Usage")
        )
        name_word = i18n.get_message("cli.name")
        name_word = (
            name_word
            if name_word != "cli.name"
            else ("名称" if lang == "zh" else "Name")
        )
        cmd_word = i18n.get_message("cli.command")
        cmd_word = (
            cmd_word
            if cmd_word != "cli.command"
            else ("命令" if lang == "zh" else "Command")
        )
        rows = [
...
class OutputFormatter:
    ...
    def format_help_usage(self) -> str:
        ...
        headers = [
            self.i18n.get_message("cli.command"),
            self.i18n.get_message("cli.description"),
        ]
        ...
        table = self.rich_formatter.create_table(
            headers, rows, title=None  # 不使用表格 title
        )
    ...
    def format_command_result(self, result) -> str:
        ...
        if (
            getattr(result, "error", None) or getattr(result, "stderr", None)
    ...
    def print_plugin_list(
        self, enabled_plugins: List[Dict], disabled_plugins: List[Dict] = None
    ):
        """打印插件列表"""
        # 使用i18n管理器获取文本
        from ..utils.i18n import get_i18n_manager
        ...
        if enabled_plugins:
            ...
            for plugin in enabled_plugins:
                ...
                plugin_type_text = plugin.get(
                    "type", i18n.get_message("plugin_types.system")
                )
                ...
                priority = self.color_helper.colorize_number(
                    str(plugin.get("priority", "")), "bright_blue"
                )
                version = f"[dim]{plugin.get('version', '')}[/dim]"
                cmd_count = self.color_helper.colorize_number(
                    str(plugin.get("command_count", 0)), "bright_green"
                )
                desc = f"[dim]{plugin.get('description', '')}[/dim]"

                rows.append(
                    [plugin_name, status, ptype, priority, version, cmd_count, desc]
                )

            total_commands = sum(
                plugin.get("command_count", 0) for plugin in enabled_plugins
            )
        ...
        if disabled_plugins:
            ...
            for plugin in disabled_plugins:
                plugin_type_text = plugin.get(
                    "type", i18n.get_message("plugin_types.third_party")
                )
                rows.append(
                    [
                        plugin.get("name", ""),
            ...
            total_commands = sum(
                plugin.get("command_count", 0) for plugin in disabled_plugins
            )
        ...
        disabled_commands = sum(
            p.get("command_count", 0) for p in (disabled_plugins or [])
        )
    ...
    def print_plugin_info(self, plugin_info: Dict):
        ...
        if commands:
            ...
            for cmd in commands:
                ...
                subplugin = self.color_helper.colorize_subplugin(
                    cmd.get("subplugin", "")
                )
                shell_func = f"[dim]{cmd.get('shell_function', '')}[/dim]"
                plugin_type = self.color_helper.colorize_type(
                    cmd.get("plugin_type", "")
                )
                usage = self.color_helper.colorize_usage(cmd.get("usage", ""))
                description = f"[dim]{cmd.get('description', '')}[/dim]"
                cmd_rows.append(
                    [command, subplugin, shell_func, plugin_type, usage, description]
                )

            count_text = (
                f"({len(commands)} {texts['commands_count']})"
                if language == "zh"
                else f"({len(commands)} {texts['commands_count']})"
            )
            commands_table_title = f"📜 {texts['available_commands']} {count_text}"

            cmd_column_styles = ["", "", "", "", "", ""]
            cmd_column_justifies = [
                "left",
                "center",
                "center",
                "center",
