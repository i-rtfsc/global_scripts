...
class PluginValidator:
    ...
    def validate_plugin_directory(self, plugin_dir: Path) -> ValidationResult:
        ...
        if not plugin_dir.is_dir():
            return ValidationResult(
                is_valid=False, error_message=f"Not a directory: {plugin_dir}"
            )
    ...
    def validate_dependencies(
        self, requirements: Dict[str, list], plugin_name: str
    ) -> ValidationResult:
        """
        验证插件依赖
        ...
        if not isinstance(requirements, dict):
            return ValidationResult(
                is_valid=False, error_message="Requirements must be a dictionary"
            )
    ...
    def validate_plugin_config(self, plugin_json_path: Path) -> ValidationResult:
        """
        验证 plugin.json 配置文件
        ...
        try:
            ...
            return ValidationResult(
                is_valid=False, error_message=f"Invalid JSON format: {e}"
            )
        except Exception as e:
            return ValidationResult(
                is_valid=False, error_message=f"Failed to read config: {e}"
            )

        # 验证配置内容
        if not isinstance(config, dict):
            return ValidationResult(
                is_valid=False, error_message="Plugin config must be a JSON object"
            )
    ...
    def validate_python_plugin(self, python_file: Path) -> ValidationResult:
        ...
        if not python_file.exists():
            return ValidationResult(
                is_valid=False, error_message=f"Python file not found: {python_file}"
            )
        ...
        try:
            ...
            return ValidationResult(
                is_valid=False, error_message=f"Python syntax error: {e}"
            )
        except Exception as e:
    ...
    def validate_shell_script(self, script_file: Path) -> ValidationResult:
        ...
        if not script_file.exists():
            return ValidationResult(
                is_valid=False, error_message=f"Script file not found: {script_file}"
            )
