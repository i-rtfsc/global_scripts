...
class BasePlugin(ABC):
    ...
    async def execute_function(self, func_name: str, args: List[str] = None) -> CommandResult:
        """执行子插件的特定函数"""
        if func_name not in self.functions:
            from gscripts.utils.i18n import I18nManager
            i18n = I18nManager()
            return CommandResult(False, error=i18n.get_message('errors.function_not_found', function=func_name, subplugin=self.name))
        ...
        if not method:
            ...
            for attr_name in dir(self):
                attr = getattr(self, attr_name)
                if hasattr(attr, '_plugin_metadata') and attr._plugin_metadata.name == func_name:
                    method = attr
                    break
        ...
        try:
            ...
            from gscripts.utils.i18n import I18nManager
            i18n = I18nManager()
            return CommandResult(False, error=i18n.get_message('errors.execution_failed', error=str(e)))
    
    async def run_shell_command(self, command: Union[str, List[str]], timeout: int = 30) -> CommandResult:
    ...
    def _discover_functions(self):
        ...
        for func_name, func_config in functions_config.items():
            if isinstance(func_config, str):
                ...
                description = func_config.get('description', f"执行命令: {command_template}")
            else:
                continue
            
            # 创建动态函数
            async def dynamic_function(args: List[str] = None, template=command_template):
...
class Plugin:
    ...
    async def execute(self, sub_plugin_name: str, func_name: str, args: List[str] = None) -> CommandResult:
        """执行插件下的子插件函数"""
        sub_plugin = self.get_sub_plugin(sub_plugin_name)
        if not sub_plugin:
            return CommandResult(False, error=f"子插件 '{sub_plugin_name}' 不存在于 {self.name}")
