...
SENSITIVE_PATTERNS = [
    (
        re.compile(
            r'(password|passwd|pwd|token|key|secret|api[_-]?key|authorization|cookie|session)[=:]\s*["\']?([^"\'\s]+)',
            re.IGNORECASE,
        ),
        r"\1=***REDACTED***",
    ),
    (
        re.compile(r"(Bearer|Basic|Token)\s+[\w\-\.=]+", re.IGNORECASE),
        r"\1 ***REDACTED***",
    ),
    ...
    (
        ...
        lambda m: f"***KEY[{len(m.group())}]***" if len(m.group()) > 40 else m.group(),
    ),
]
...
def duration(start_time: float) -> int:
    ...
    return (
        ...
        else int((time.time() - start_time) * 1000)
    )
...
def log_context(**kwargs) -> Dict[str, Any]:
    ...
    Returns:
        包含标准字段的上下文字典
    """
    context = {"correlation_id": correlation_id(), "timestamp": time.time()}
