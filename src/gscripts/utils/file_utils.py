...
class FileUtils:
    ...
    async def read_text_async(file_path: Union[str, Path], encoding: str = 'utf-8') -> str:
        ...
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug(f"cid={cid} read_text_async.leave path={file_path} size={len(content)}")
        return content
    ...
    async def write_text_async(
        ...
        cid = correlation_id()
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug(f"cid={cid} write_text_async.enter path={file_path} size={len(content)}")
        # 确保父目录存在
    ...
    def read_json(file_path: Union[str, Path]) -> Dict[str, Any]:
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            data = json.load(f)
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug(f"cid={cid} read_json path={file_path} keys={len(data) if isinstance(data, dict) else 'n/a'}")
        return data
    ...
    async def read_json_async(file_path: Union[str, Path]) -> Dict[str, Any]:
        ...
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug(f"cid={cid} read_json_async path={file_path} keys={len(data) if isinstance(data, dict) else 'n/a'}")
        return data
    ...
    def write_json(file_path: Union[str, Path], data: Dict[str, Any], indent: int = 2) -> None:
        ...
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug(f"cid={cid} write_json path={file_path} keys={len(data) if isinstance(data, dict) else 'n/a'}")
    ...
    async def write_json_async(
        ...
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug(f"cid={cid} write_json_async path={file_path} keys={len(data) if isinstance(data, dict) else 'n/a'}")
    ...
    def find_files(
        ...
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug(f"find_files dir={directory} pattern={pattern} recursive={recursive} count={len(files)}")
        return files
    ...
    async def load_config_file(file_path: Union[str, Path]) -> Optional[Dict[str, Any]]:
        ...
        if not Path(file_path).exists():
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(f"cid={cid} load_config_file.skip_missing path={file_path}")
            return None
        ...
        try:
            if ext == '.json':
                ...
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug(f"cid={cid} load_config_file.success path={file_path} keys={len(data) if isinstance(data, dict) else 'n/a'}")
                return data
            else:
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug(f"cid={cid} load_config_file.unsupported path={file_path} ext={ext}")
                return None
        except Exception as e:
            logger.warning(f"cid={cid} load_config_file.error path={file_path} ext={ext} error={type(e).__name__}: {e}")
            return None
    ...
    def safe_filename(filename: str) -> str:
        ...
        cid = correlation_id()
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug(f"cid={cid} safe_filename.enter original={safe_repr(filename)} length={len(filename)}")
        # 移除或替换危险字符
        ...
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug(f"cid={cid} safe_filename.leave original_length={len(filename)} result={safe_repr(result)}")
        return result