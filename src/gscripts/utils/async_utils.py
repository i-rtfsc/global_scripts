...
class AsyncFileUtils:
    ...
    @staticmethod
    async def write_text(file_path: Path, content: str, encoding: str = 'utf-8') -> None:
        """异步写入文本文件"""
        loop = asyncio.get_event_loop()
        with concurrent.futures.ThreadPoolExecutor() as executor:
            return await loop.run_in_executor(
...
class AsyncUtils:
    ...
    @staticmethod
    async def execute_command_async(command: str, cwd: Optional[str] = None,
    ...
    @staticmethod
    async def batch_execute_async(commands: List[str], max_concurrent: int = 5,
