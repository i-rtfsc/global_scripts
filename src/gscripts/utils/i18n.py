...
class I18nManager:
    ...
    def __init__(self, config_path: Optional[Path] = None, chinese: bool = None):
        if config_path is None:
            ...
            # 新结构: 使用包内资源文件
            config_path = (
                Path(__file__).parent.parent / "resources" / "i18n" / "i18n.json"
            )
        ...
        try:
            if self.config_path.exists():
                with open(self.config_path, "r", encoding="utf-8") as f:
                    ...
                    self.current_language = os.environ.get(
                        "GS_LANGUAGE", data.get("locale", {}).get("default", "en")
                    )
    ...
    def get_message(self, key: str, **kwargs) -> str:
        ...
        if isinstance(node, dict):
            message = (
                node.get(self.current_language) or node.get("en") or node.get("zh")
            )
