...
def parseargs():
    ...
    buildoptiongroup.add_option("-b", "--branch", dest="branch",
                                help="what remote branch", default="flyme10_base")
    buildoptiongroup.add_option("-f", "--file", dest="file",
                                help="project list", default="project_review.list")
    buildoptiongroup.add_option("-u", "--user", dest="user",
                                help="user", default="x-huanganqi")
    buildoptiongroup.add_option("-g", "--review", dest="review",
                                help="review", default="@review.rnd.meizu.com")
...
def parse_file(file):
    ...
    with open(file, 'r') as f:
        for line in f.readlines():
            ...
            project.branch = line.split(",")[2].strip()
...
def init(env):
    ...
    for pro in projects:
        ...
        if dir_exists(os.path.join(git_dir, ".git")):
            ...
            git_cmd = "git clone \"http://{user}{review}/a/aosp/app/Client/{project}\" -b {branch} {dir}"
            cmd = git_cmd.format(user=env.user, review=env.review, project=project, dir=dir, branch=branch)
            print(cmd)
            os.system(cmd)
...
def main():
    ...
    env.review = options.review.strip()
