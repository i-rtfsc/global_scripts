...
def parseargs():
    ...
    buildoptiongroup.add_option("-b", "--branch", dest="branch",
                                help="what remote branch", default="androidt")
    buildoptiongroup.add_option("-f", "--file", dest="file",
                                help="project list", default="project_upuphone.list")
    buildoptiongroup.add_option("-u", "--user", dest="user",
                                help="user", default="anqi.huang")
    buildoptiongroup.add_option("-g", "--gerrit", dest="gerrit",
                                help="gerrit", default="@gerrit.upuphone.com")
...
def init(env):
    ...
    for pro in projects:
        ...
        if dir_exists(os.path.join(git_dir, ".git")):
            ...
            git_cmd = "git clone \"ssh://{user}{gerrit}:29418/{project}\" -b {branch} {dir} && scp -p -P 29418 {user}{gerrit}:hooks/commit-msg \"{dir}/.git/hooks/\""
