# Global Scripts V3 系统架构设计

## 📐 架构概述

### 1.1 设计理念
Global Scripts V3 采用现代化的分层架构设计，去除V2兼容性包袱，保留 `custom/global_scripts-refactor-fail` 中的优秀架构思想，打造简洁高效的Shell开发工具框架。

### 1.2 核心原则
- **简洁性优先**: 依赖最小化，只需shell/python环境
- **分层解耦**: 清晰的层次边界，单向依赖关系
- **插件化扩展**: 类似oh-my-zsh的本地插件机制
- **配置驱动**: JSON Schema驱动的现代化配置管理
- **性能导向**: 启动快速，运行高效，缓存优化

## 🏗️ 五层架构设计

### 2.1 架构层次图

```
┌─────────────────────────────────────────────────────┐
│               用户接口层 (User Interface Layer)       │
│                                                    │
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐    │
│  │ CLI命令接口 │ │ Tab补全系统 │ │ 帮助文档    │    │
│  │             │ │             │ │             │    │
│  │ • gs-*命令  │ │ • 智能补全  │ │ • 分层帮助  │    │
│  │ • 参数解析  │ │ • 历史记录  │ │ • 示例展示  │    │
│  │ • 格式输出  │ │ • 缓存优化  │ │ • 搜索功能  │    │
│  └─────────────┘ └─────────────┘ └─────────────┘    │
├─────────────────────────────────────────────────────┤
│               命令处理层 (Command Processing Layer)   │
│                                                    │
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐    │
│  │ 命令路由器  │ │ 参数处理器  │ │ 结果格式化  │    │
│  │             │ │             │ │             │    │
│  │ • 动态分发  │ │ • 类型转换  │ │ • JSON输出  │    │
│  │ • 别名展开  │ │ • 验证检查  │ │ • 表格格式  │    │
│  │ • 权限检查  │ │ • 默认值   │ │ • 管道支持  │    │
│  └─────────────┘ └─────────────┘ └─────────────┘    │
├─────────────────────────────────────────────────────┤
│                插件服务层 (Plugin Service Layer)      │
│                                                    │
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐    │
│  │ 插件管理器  │ │ 插件加载器  │ │ 插件API    │    │
│  │             │ │             │ │             │    │
│  │ • 发现扫描  │ │ • 懒加载    │ │ • 统一接口  │    │
│  │ • 状态管理  │ │ • 依赖解析  │ │ • 钩子系统  │    │
│  │ • 启用禁用  │ │ • 沙箱执行  │ │ • 事件通知  │    │
│  └─────────────┘ └─────────────┘ └─────────────┘    │
├─────────────────────────────────────────────────────┤
│              核心服务层 (Core Service Layer)          │
│                                                    │
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐    │
│  │ 配置管理    │ │ 缓存系统    │ │ 性能监控    │    │
│  │             │ │             │ │             │    │
│  │ • JSON配置  │ │ • L1内存缓存│ │ • 启动监控  │    │
│  │ • Schema验证│ │ • L2磁盘缓存│ │ • 性能统计  │    │
│  │ • 热重载    │ │ • 缓存策略  │ │ • 瓶颈分析  │    │
│  └─────────────┘ └─────────────┘ └─────────────┘    │
├─────────────────────────────────────────────────────┤
│              基础设施层 (Infrastructure Layer)        │
│                                                    │
│  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐    │
│  │ 日志系统    │ │ 错误处理    │ │ 工具库      │    │
│  │             │ │             │ │             │    │
│  │ • 多级日志  │ │ • 统一错误  │ │ • 跨平台    │    │
│  │ • 彩色输出  │ │ • 错误恢复  │ │ • 工具函数  │    │
│  │ • 文件轮转  │ │ • 诊断建议  │ │ • 系统检测  │    │
│  └─────────────┘ └─────────────┘ └─────────────┘    │
└─────────────────────────────────────────────────────┘
```

### 2.2 层次职责定义

#### 用户接口层 (User Interface Layer)
**职责**: 提供用户交互接口，处理用户输入输出
- **CLI命令接口**: 实现所有 `gs-*` 命令的用户接口
- **Tab补全系统**: 智能的命令和参数补全功能
- **帮助文档系统**: 分层次的帮助信息和示例展示

#### 命令处理层 (Command Processing Layer)  
**职责**: 处理命令解析、验证和结果格式化
- **命令路由器**: 根据命令名称分发到对应的处理函数
- **参数处理器**: 解析、转换和验证命令参数
- **结果格式化器**: 将执行结果格式化为用户友好的输出

#### 插件服务层 (Plugin Service Layer)
**职责**: 管理插件生命周期，提供插件运行环境
- **插件管理器**: 发现、启用、禁用插件，管理插件状态
- **插件加载器**: 懒加载插件，解析依赖关系
- **插件API**: 为插件提供统一的接口和钩子系统

#### 核心服务层 (Core Service Layer)
**职责**: 提供系统核心服务和基础功能
- **配置管理**: JSON Schema驱动的配置系统
- **缓存系统**: 双级缓存优化系统性能
- **性能监控**: 监控系统性能，提供优化建议

#### 基础设施层 (Infrastructure Layer)
**职责**: 提供底层技术服务和跨平台支持
- **日志系统**: 结构化日志记录和管理
- **错误处理**: 统一的错误处理和恢复机制
- **工具库**: 通用工具函数和跨平台适配

## 🔧 技术栈架构

### 3.1 核心技术栈

#### Shell脚本层 (必需依赖)
```bash
# Shell环境要求
supported_shells=(
    "bash 4.0+"     # 主要支持Shell
    "zsh 5.0+"      # 次要支持Shell
)

# Shell负责的功能
shell_responsibilities=(
    "系统命令调用"
    "环境变量管理"
    "进程生命周期管理"
    "用户接口展示"
    "文件系统操作"
)
```

#### Python增强层 (可选依赖)
```bash
# Python环境要求
python_version="2.7+"

# Python负责的功能  
python_responsibilities=(
    "JSON Schema验证"
    "复杂数据处理"
    "依赖关系解析"
    "配置文件处理"
    "性能数据分析"
)
```

#### 系统工具层 (标准工具)
```bash
# 必需的系统工具
required_tools=(
    "jq"            # JSON处理
    "curl"          # HTTP请求
    "git"           # 版本控制
    "grep/sed/awk"  # 文本处理
)

# 可选的增强工具
optional_tools=(
    "fzf"           # 模糊搜索
    "bat"           # 语法高亮
    "fd"            # 快速文件查找
    "ripgrep"       # 快速文本搜索
)
```

### 3.2 可选高性能插件

#### Go语言插件 (可选)
```bash
# Go插件示例
go_plugins=(
    "gs-performance-monitor"    # 高性能监控插件
    "gs-cache-engine"          # 高速缓存引擎
    "gs-concurrent-executor"   # 并发执行引擎
)

# 安装方式
install_go_plugin() {
    echo "检测到Go环境，安装高性能插件..."
    go install github.com/globalscripts/performance-monitor@latest
}
```

#### Rust语言插件 (可选)
```bash
# Rust插件示例
rust_plugins=(
    "gs-security-sandbox"      # 安全沙箱插件
    "gs-system-monitor"        # 系统监控插件
    "gs-crypto-tools"          # 加密工具插件
)

# 安装方式
install_rust_plugin() {
    echo "检测到Rust环境，安装安全增强插件..."
    cargo install gs-security-sandbox
}
```

### 3.3 依赖管理策略

#### 分层依赖检查
```bash
# 依赖检查函数
check_dependencies() {
    local level="$1"
    
    case "$level" in
        "required")
            check_required_dependencies
            ;;
        "optional")
            check_optional_dependencies
            ;;
        "plugin")
            check_plugin_dependencies
            ;;
    esac
}

# 必需依赖检查
check_required_dependencies() {
    local missing_deps=()
    
    # 检查Shell环境
    if [[ -z "$BASH_VERSION" && -z "$ZSH_VERSION" ]]; then
        missing_deps+=("bash或zsh")
    fi
    
    # 检查Python环境
    if ! command -v python3 &>/dev/null; then
        missing_deps+=("python3")
    fi
    
    # 检查jq
    if ! command -v jq &>/dev/null; then
        missing_deps+=("jq")
    fi
    
    if [[ ${#missing_deps[@]} -gt 0 ]]; then
        echo "错误: 缺少必需依赖: ${missing_deps[*]}"
        return 1
    fi
}
```

## 📦 模块设计

### 4.1 核心模块架构

#### 系统引导模块 (core/bootstrap.sh)
```bash
# 引导流程设计
bootstrap_sequence() {
    # 1. 环境检测
    detect_environment
    
    # 2. 依赖检查
    check_dependencies "required"
    
    # 3. 性能监控启动
    start_performance_monitoring
    
    # 4. 配置系统初始化
    initialize_configuration
    
    # 5. 缓存系统启动
    initialize_cache_system
    
    # 6. 插件系统初始化
    initialize_plugin_system
    
    # 7. 命令注册
    register_core_commands
    
    # 8. 完成启动
    finalize_bootstrap
}

# 环境检测
detect_environment() {
    # 检测操作系统
    detect_os_type
    
    # 检测Shell类型
    detect_shell_type
    
    # 检测Python版本
    detect_python_version
    
    # 设置环境变量
    setup_environment_variables
}
```

#### 配置管理模块 (core/config.sh)
```bash
# 配置系统架构
config_system_architecture() {
    # 配置文件层次
    configuration_hierarchy=(
        "$HOME/.gs/config.json"         # 用户配置
        "$GS_ROOT/config/default.json"  # 默认配置
        "$GS_ROOT/config/system.json"   # 系统配置
    )
    
    # 配置合并策略
    merge_strategy="user_override_default"
    
    # 配置验证
    schema_validation="enabled"
    
    # 配置缓存
    config_cache="memory_cache_with_file_watch"
}

# 配置API设计
declare -gA GS_CONFIG_CACHE=()

gs_config_get() {
    local key="$1"
    local default_value="$2"
    
    # 检查缓存
    if [[ -n "${GS_CONFIG_CACHE[$key]}" ]]; then
        echo "${GS_CONFIG_CACHE[$key]}"
        return 0
    fi
    
    # 从配置文件读取
    local value
    value=$(jq -r ".$key // empty" "$HOME/.gs/config.json" 2>/dev/null)
    
    if [[ -n "$value" && "$value" != "null" ]]; then
        # 更新缓存
        GS_CONFIG_CACHE[$key]="$value"
        echo "$value"
    else
        echo "$default_value"
    fi
}
```

#### 插件管理模块 (core/loader.sh)
```bash
# 插件系统架构
plugin_system_architecture() {
    # 插件发现路径
    plugin_paths=(
        "$GS_ROOT/plugins"              # 内置插件
        "$HOME/.gs/custom"              # 用户自定义插件
    )
    
    # 插件加载策略
    loading_strategy="lazy_loading_with_dependency_resolution"
    
    # 插件状态管理
    state_management="in_memory_with_persistence"
    
    # 插件沙箱
    sandbox_mode="lightweight_isolation"
}

# 插件发现和注册
declare -gA GS_PLUGIN_REGISTRY=()
declare -gA GS_PLUGIN_MANIFESTS=()
declare -gA GS_PLUGIN_STATE=()

gs_plugin_discover() {
    local plugin_dir="$1"
    local manifest_file="$plugin_dir/manifest.json"
    
    # 检查manifest文件
    [[ -f "$manifest_file" ]] || return 1
    
    # 验证manifest格式
    if ! jq -e . "$manifest_file" >/dev/null 2>&1; then
        gs_log "WARN" "插件manifest格式错误: $manifest_file"
        return 1
    fi
    
    # 提取插件信息
    local plugin_name
    plugin_name=$(jq -r '.name' "$manifest_file")
    
    # 注册插件
    GS_PLUGIN_REGISTRY["$plugin_name"]="$plugin_dir"
    GS_PLUGIN_MANIFESTS["$plugin_name"]="$manifest_file"
    GS_PLUGIN_STATE["$plugin_name"]="discovered"
    
    gs_log "DEBUG" "发现插件: $plugin_name"
}
```

### 4.2 插件架构设计

#### 分层插件Manifest规范
```json
{
  "$schema": "https://globalscripts.dev/schema/plugin-manifest-v3.json",
  "name": "android",
  "version": "3.0.0",
  "description": "Android开发工具集",
  "type": "main",
  "author": {
    "name": "Global Scripts",
    "email": "dev@globalscripts.dev"
  },
  "main": "android.sh",
  "dependencies": {
    "system": ["adb", "fastboot"],
    "plugins": [],
    "python": []
  },
  "submodules": {
    "adb": {
      "name": "Android Debug Bridge",
      "description": "ADB工具集",
      "manifest": "adb/manifest.json",
      "main": "adb/adb_tools.sh"
    },
    "fastboot": {
      "name": "Android Fastboot",
      "description": "Fastboot刷机工具",
      "manifest": "fastboot/manifest.json", 
      "main": "fastboot/fastboot_tools.sh"
    },
    "perfetto": {
      "name": "Android Performance Analysis",
      "description": "Android系统性能分析工具",
      "manifest": "perfetto/manifest.json",
      "main": "perfetto/perfetto_tools.sh"
    }
  },
  "commands": {
    "gs-android-device-list": {
      "function": "android_device_list",
      "description": "列出连接的Android设备",
      "module": "main",
      "aliases": ["adl"]
    },
    "gs-android-adb-shell": {
      "function": "adb_shell_connect", 
      "description": "连接ADB Shell",
      "module": "adb",
      "aliases": ["ash"]
    },
    "gs-android-fastboot-devices": {
      "function": "fastboot_list_devices",
      "description": "列出Fastboot设备",
      "module": "fastboot",
      "aliases": ["fbd"]
    }
  },
  "config": {
    "android.adb_timeout": {
      "type": "integer",
      "default": 30,
      "description": "ADB命令超时时间(秒)"
    },
    "android.fastboot_timeout": {
      "type": "integer", 
      "default": 60,
      "description": "Fastboot命令超时时间(秒)"
    }
  },
  "completion": {
    "auto_generate": true,
    "custom_completion": "completion/android_completion.sh"
  },
  "hooks": {
    "on_load": "android_plugin_init",
    "on_unload": "android_plugin_cleanup",
    "on_config_change": "android_config_update"
  }
}
```

#### 子模块Manifest示例 (adb/manifest.json)
```json
{
  "$schema": "https://globalscripts.dev/schema/submodule-manifest-v3.json",
  "name": "adb",
  "parent": "android",
  "version": "3.0.0",
  "description": "Android Debug Bridge工具集",
  "type": "submodule",
  "main": "adb_tools.sh",
  "dependencies": {
    "system": ["adb"]
  },
  "commands": {
    "gs-android-adb-shell": {
      "function": "adb_shell_connect",
      "description": "连接到设备ADB Shell",
      "completion": "adb_shell_completion"
    },
    "gs-android-adb-install": {
      "function": "adb_install_apk", 
      "description": "安装APK到设备",
      "completion": "adb_install_completion"
    },
    "gs-android-adb-push": {
      "function": "adb_push_file",
      "description": "推送文件到设备",
      "completion": "adb_push_completion"
    },
    "gs-android-adb-pull": {
      "function": "adb_pull_file",
      "description": "从设备拉取文件", 
      "completion": "adb_pull_completion"
    }
  },
  "config": {
    "adb.default_timeout": {
      "type": "integer",
      "default": 30,
      "description": "默认ADB操作超时时间"
    },
    "adb.preferred_device": {
      "type": "string",
      "default": "",
      "description": "首选设备ID（为空时自动选择）"
    }
  }
}
```

#### 分层插件管理系统
```bash
# 插件目录结构 - 支持子模块分层
plugins/
├── android/                    # Android开发工具插件
│   ├── manifest.json          # 主插件清单
│   ├── android.sh             # 主入口文件
│   ├── adb/                   # ADB工具子模块
│   │   ├── manifest.json      # ADB子模块清单
│   │   └── adb_tools.sh       # ADB功能实现
│   ├── fastboot/              # Fastboot子模块
│   │   ├── manifest.json
│   │   └── fastboot_tools.sh
│   ├── perfetto/              # 性能分析子模块
│   │   ├── manifest.json
│   │   └── perfetto_tools.sh
│   └── README.md              # 插件使用文档
├── git/                       # Git增强插件
│   ├── manifest.json          # 主插件清单
│   ├── git.sh                 # Git主入口
│   ├── branch/                # 分支管理子模块
│   │   ├── manifest.json
│   │   └── branch_tools.sh
│   ├── commit/                # 提交管理子模块
│   │   ├── manifest.json
│   │   └── commit_tools.sh
│   └── README.md
└── system/                    # 系统管理插件
    ├── manifest.json          # 主插件清单
    ├── system.sh              # 系统主入口
    ├── proxy/                 # 代理管理子模块
    │   ├── manifest.json
    │   └── proxy_tools.sh
    ├── network/               # 网络工具子模块
    │   ├── manifest.json
    │   └── network_tools.sh
    └── README.md

custom/ # 根目录的.gitignore忽略掉这个目录
├── README.md                  # 插件开发说明
└── my_plugin/                 # 用户插件示例
    ├── manifest.json          # 支持子模块的插件清单
    ├── my_plugin.sh           # 主入口
    └── tools/                 # 工具子模块
        ├── manifest.json
        └── custom_tools.sh
```

#### 自动Tab补全生成系统
```bash
# 补全系统自动化设计 (completion/completion_generator.sh)

# 从manifest自动生成补全脚本
gs_generate_completion() {
    local plugin_name="$1"
    local plugin_dir="${GS_PLUGIN_REGISTRY[$plugin_name]}"
    local manifest_file="$plugin_dir/manifest.json"
    
    # 解析主插件manifest
    local commands
    commands=$(jq -r '.commands | keys[]' "$manifest_file")
    
    # 生成主插件补全
    generate_plugin_completion "$plugin_name" "$commands"
    
    # 解析并生成子模块补全
    local submodules
    submodules=$(jq -r '.submodules // {} | keys[]' "$manifest_file" 2>/dev/null)
    
    for submodule in $submodules; do
        local submodule_manifest="$plugin_dir/$submodule/manifest.json"
        if [[ -f "$submodule_manifest" ]]; then
            local sub_commands
            sub_commands=$(jq -r '.commands | keys[]' "$submodule_manifest")
            generate_submodule_completion "$plugin_name" "$submodule" "$sub_commands"
        fi
    done
}

# 生成插件命令补全
generate_plugin_completion() {
    local plugin_name="$1"
    local commands="$2"
    
    for command in $commands; do
        local completion_func="${command//-/_}_completion"
        
        # 检查是否有自定义补全函数
        if declare -F "$completion_func" >/dev/null 2>&1; then
            register_custom_completion "$command" "$completion_func"
        else
            # 自动生成基础补全
            generate_auto_completion "$command" "$plugin_name"
        fi
    done
}

# 自动生成基础补全逻辑
generate_auto_completion() {
    local command="$1" 
    local plugin_name="$2"
    local manifest_file="${GS_PLUGIN_REGISTRY[$plugin_name]}/manifest.json"
    
    # 从manifest提取命令参数信息
    local command_info
    command_info=$(jq -r ".commands[\"$command\"]" "$manifest_file")
    
    # 生成参数补全
    local completion_script="
_${command//-/_}_completion() {
    local cur=\${COMP_WORDS[COMP_CWORD]}
    local prev=\${COMP_WORDS[COMP_CWORD-1]}
    
    case \"\$prev\" in"
    
    # 添加通用参数补全
    completion_script+="
        --format)
            COMPREPLY=(\$(compgen -W 'json text table' -- \"\$cur\"))
            return 0
            ;;
        --config)
            COMPREPLY=(\$(compgen -f -- \"\$cur\"))
            return 0
            ;;"
    
    # 从manifest添加特定参数补全
    local params
    params=$(echo "$command_info" | jq -r '.parameters // {} | keys[]' 2>/dev/null)
    
    for param in $params; do
        local param_type
        param_type=$(echo "$command_info" | jq -r ".parameters[\"$param\"].type" 2>/dev/null)
        
        case "$param_type" in
            "file")
                completion_script+="
        --$param)
            COMPREPLY=(\$(compgen -f -- \"\$cur\"))
            return 0
            ;;"
                ;;
            "directory")
                completion_script+="
        --$param)
            COMPREPLY=(\$(compgen -d -- \"\$cur\"))
            return 0
            ;;"
                ;;
        esac
    done
    
    completion_script+="
    esac
    
    # 默认选项补全
    local opts='--help --verbose --quiet --json --format'
    COMPREPLY=(\$(compgen -W \"\$opts\" -- \"\$cur\"))
}

complete -F _${command//-/_}_completion $command"
    
    # 写入补全脚本文件
    echo "$completion_script" > "$GS_COMPLETION_DIR/${command}.completion"
}

# 插件加载时自动注册补全
gs_plugin_register_completion() {
    local plugin_name="$1"
    
    # 生成补全脚本
    gs_generate_completion "$plugin_name"
    
    # 加载补全脚本
    local completion_files="$GS_COMPLETION_DIR/${plugin_name}_*.completion"
    for completion_file in $completion_files; do
        [[ -f "$completion_file" ]] && source "$completion_file"
    done
    
    gs_log "DEBUG" "已注册插件补全: $plugin_name"
}

# 子模块补全支持
generate_submodule_completion() {
    local plugin_name="$1"
    local submodule="$2" 
    local commands="$3"
    
    for command in $commands; do
        # 子模块命令通常以父插件名开头
        if [[ "$command" == gs-${plugin_name}-* ]]; then
            generate_auto_completion "$command" "$plugin_name"
        fi
    done
}

# 动态补全数据获取
get_dynamic_completion_data() {
    local command="$1"
    local current_word="$2"
    
    case "$command" in
        "gs-android-device-list"|"gs-android-adb-shell")
            # 动态获取Android设备列表
            adb devices 2>/dev/null | awk 'NR>1 && $2=="device" {print $1}'
            ;;
        "gs-git-branch-create"|"gs-git-branch-delete")
            # 动态获取Git分支列表
            git branch 2>/dev/null | sed 's/^[* ] //'
            ;;
        "gs-plugins-enable"|"gs-plugins-disable")
            # 动态获取插件列表
            gs-plugins-list --format json 2>/dev/null | jq -r '.[].name'
            ;;
    esac
}
```

# 插件生命周期钩子
gs_plugin_hook() {
    local hook_name="$1"
    local plugin_name="$2"
    shift 2
    local hook_args=("$@")
    
    local manifest_file="${GS_PLUGIN_MANIFESTS[$plugin_name]}"
    local hook_function
    hook_function=$(jq -r ".hooks.$hook_name // empty" "$manifest_file")
    
    if [[ -n "$hook_function" ]] && declare -F "$hook_function" &>/dev/null; then
        gs_log "DEBUG" "执行插件钩子: $plugin_name.$hook_function"
        "$hook_function" "${hook_args[@]}"
    fi
}

# 插件命令注册
gs_plugin_register_command() {
    local command_name="$1"
    local function_name="$2"
    local plugin_name="$3"
    
    GS_COMMAND_REGISTRY["$command_name"]="$plugin_name:$function_name"
    gs_log "DEBUG" "注册插件命令: $command_name -> $plugin_name:$function_name"
}

# 插件配置访问
gs_plugin_config_get() {
    local plugin_name="$1"
    local config_key="$2"
    local default_value="$3"
    
    gs_config_get "plugins.$plugin_name.$config_key" "$default_value"
}
```

## 🎯 命令接口设计

### 5.1 命令规范

#### 命名规范
```bash
# 命令命名模式
gs-<category>-<action>[-<target>]

# 示例
gs-help                        # 系统命令
gs-version                     # 系统命令
gs-config-get                  # 配置命令
gs-plugins-list                # 插件命令
gs-android-device-list         # Android插件命令
gs-git-status-enhanced         # Git插件命令
```

#### 参数规范
```bash
# 通用参数
--help, -h                     # 显示帮助信息
--verbose, -v                  # 详细输出模式
--quiet, -q                    # 静默模式
--json                         # JSON格式输出
--format <fmt>                 # 指定输出格式
--config <file>                # 指定配置文件
```

### 5.2 核心命令接口

#### 帮助系统 (gs-help)
```bash
# 帮助命令设计
gs-help [command]              # 显示帮助信息

# 使用示例
gs-help                        # 显示总体帮助
gs-help config                 # 显示配置相关命令帮助
gs-help gs-android-device-list # 显示特定命令帮助

# 帮助信息结构
help_structure() {
    cat << 'EOF'
Global Scripts V3 - 现代化Shell开发工具框架

用法: gs-<command> [options] [arguments]

核心命令:
  help                显示帮助信息
  version             显示版本信息
  status              显示系统状态

配置管理:
  config-get          获取配置值
  config-set          设置配置值
  config-list         列出所有配置
  config-validate     验证配置文件

插件管理:
  plugins-list        列出插件
  plugins-enable      启用插件
  plugins-disable     禁用插件
  plugins-info        查看插件信息

工具命令:
  tools-health        系统健康检查
  tools-performance   性能分析
  tools-cache         缓存管理

使用 'gs-help <command>' 查看具体命令的详细帮助
EOF
}
```

#### 版本信息 (gs-version)
```bash
# 版本命令设计
gs-version [--json] [--detailed]

# 实现示例
gs_version() {
    local format="text"
    local detailed="false"
    
    while [[ $# -gt 0 ]]; do
        case $1 in
            --json)
                format="json"
                shift
                ;;
            --detailed)
                detailed="true"
                shift
                ;;
            *)
                gs_error "未知参数: $1"
                ;;
        esac
    done
    
    if [[ "$format" == "json" ]]; then
        output_version_json "$detailed"
    else
        output_version_text "$detailed"
    fi
}

# JSON格式输出
output_version_json() {
    local detailed="$1"
    
    local version_info='{
        "version": "'$(cat "$GS_ROOT/VERSION")'",
        "build_date": "'$(date -r "$GS_ROOT/VERSION" '+%Y-%m-%d')'",
        "shell": "'$0'",
        "platform": "'$(uname -s)'",
        "architecture": "'$(uname -m)'"'
    
    if [[ "$detailed" == "true" ]]; then
        version_info+=',
        "dependencies": {
            "python": "'$(python3 --version 2>&1 | cut -d' ' -f2)'",
            "jq": "'$(jq --version 2>&1)'",
            "git": "'$(git --version 2>&1 | cut -d' ' -f3)'"
        },
        "plugins": ['
        
        # 添加插件信息
        local first=true
        for plugin in "${!GS_PLUGIN_REGISTRY[@]}"; do
            [[ "$first" == "true" ]] && first=false || version_info+=','
            local plugin_version
            plugin_version=$(jq -r '.version // "unknown"' "${GS_PLUGIN_MANIFESTS[$plugin]}")
            version_info+='"'$plugin'": "'$plugin_version'"'
        done
        
        version_info+=']'
    fi
    
    version_info+='}'
    echo "$version_info" | jq .
}
```

#### 系统状态 (gs-status)
```bash
# 状态命令设计
gs-status [--summary] [--json] [--detailed]

# 状态检查实现
gs_status() {
    local format="text"
    local mode="full"
    
    while [[ $# -gt 0 ]]; do
        case $1 in
            --summary)
                mode="summary"
                shift
                ;;
            --json)
                format="json"
                shift
                ;;
            --detailed)
                mode="detailed"
                shift
                ;;
            *)
                gs_error "未知参数: $1"
                ;;
        esac
    done
    
    case "$mode" in
        "summary")
            show_status_summary "$format"
            ;;
        "detailed")
            show_status_detailed "$format"
            ;;
        *)
            show_status_full "$format"
            ;;
    esac
}

# 完整状态显示
show_status_full() {
    local format="$1"
    
    if [[ "$format" == "json" ]]; then
        show_status_json
    else
        show_status_text
    fi
}

show_status_text() {
    cat << EOF
Global Scripts V3 系统状态
==========================

系统信息:
  版本: $(cat "$GS_ROOT/VERSION")
  平台: $(uname -s) $(uname -m)
  Shell: $0

运行状态:
  启动时间: ${GS_STARTUP_TIME:-未知}ms
  内存使用: $(ps -o rss= -p $$ | awk '{print $1/1024 "MB"}')
  进程ID: $$

配置状态:
  配置文件: $(test -f "$HOME/.gs/config.json" && echo "✓ 存在" || echo "✗ 不存在")
  配置有效: $(gs_config_validate >/dev/null 2>&1 && echo "✓ 有效" || echo "✗ 无效")

插件状态:
$(for plugin in "${!GS_PLUGIN_REGISTRY[@]}"; do
    local status="${GS_PLUGIN_STATE[$plugin]:-unknown}"
    printf "  %-20s %s\n" "$plugin:" "$status"
done)

缓存状态:
  L1缓存: ${#GS_CACHE_L1[@]} 条目
  L2缓存: $(find "$HOME/.gs/cache" -name "*.cache" 2>/dev/null | wc -l) 文件

依赖检查:
$(check_dependencies_status)
EOF
}
```

### 5.3 配置管理接口

#### 配置操作命令
```bash
# 配置管理命令集
gs-config-get <key> [default]           # 获取配置值
gs-config-set <key> <value>             # 设置配置值
gs-config-list [pattern]                # 列出配置项
gs-config-init                          # 初始化配置
gs-config-validate [file]               # 验证配置
gs-config-reset [key]                   # 重置配置
gs-config-backup [file]                 # 备份配置
gs-config-restore <file>                # 恢复配置
gs-config-merge <file>                  # 合并配置
gs-config-diff                          # 配置差异
gs-config-schema [operation]            # Schema操作

# 配置获取实现
gs_config_get_cmd() {
    local key="$1"
    local default_value="$2"
    
    if [[ -z "$key" ]]; then
        # 显示所有配置
        jq . "$HOME/.gs/config.json" 2>/dev/null || echo "{}"
        return 0
    fi
    
    local value
    value=$(gs_config_get "$key" "$default_value")
    echo "$value"
}

# 配置设置实现
gs_config_set_cmd() {
    local key="$1"
    local value="$2"
    
    [[ -n "$key" && -n "$value" ]] || {
        gs_error "用法: gs-config-set <key> <value>"
    }
    
    # 验证key格式
    if [[ ! "$key" =~ ^[a-zA-Z_][a-zA-Z0-9_]*(\.[a-zA-Z_][a-zA-Z0-9_]*)*$ ]]; then
        gs_error "无效的配置键: $key"
    fi
    
    # 设置配置
    gs_config_set "$key" "$value"
    
    echo "配置已更新: $key = $value"
}

# 配置验证实现
gs_config_validate_cmd() {
    local config_file="${1:-$HOME/.gs/config.json}"
    
    if [[ ! -f "$config_file" ]]; then
        gs_error "配置文件不存在: $config_file"
    fi
    
    # JSON格式检查
    if ! jq . "$config_file" >/dev/null 2>&1; then
        gs_error "配置文件JSON格式错误"
    fi
    
    # Schema验证
    if command -v python3 &>/dev/null; then
        python3 "$GS_ROOT/tools/config_validator.py" "$config_file"
    else
        echo "警告: 无Python环境，跳过Schema验证"
    fi
    
    echo "配置文件验证通过: $config_file"
}
```

### 5.4 插件管理接口

#### 插件操作命令
```bash
# 插件管理命令集
gs-plugins-list [--enabled|--available] [--format json]  # 列出插件
gs-plugins-info <plugin>                                 # 插件详情（比如插件使用命令）
gs-plugins-enable <plugin> [...]                         # 启用插件
gs-plugins-disable <plugin> [...]                        # 禁用插件
gs-plugins-reload <plugin|--all>                         # 重载插件
gs-plugins-validate <plugin>                             # 验证插件
gs-plugins-search <pattern>                              # 搜索插件
gs-plugins-create <name>                                 # 创建插件
gs-plugins-install <path>                                # 安装插件
gs-plugins-uninstall <plugin>                            # 卸载插件

# 插件列表实现
gs_plugins_list_cmd() {
    local filter="all"
    local format="text"
    
    while [[ $# -gt 0 ]]; do
        case $1 in
            --enabled)
                filter="enabled"
                shift
                ;;
            --available)
                filter="available"
                shift
                ;;
            --format)
                format="$2"
                shift 2
                ;;
            *)
                gs_error "未知参数: $1"
                ;;
        esac
    done
    
    case "$format" in
        "json")
            output_plugins_json "$filter"
            ;;
        *)
            output_plugins_text "$filter"
            ;;
    esac
}

# 文本格式输出插件列表
output_plugins_text() {
    local filter="$1"
    
    printf "%-20s %-10s %-40s\n" "插件名称" "状态" "描述"
    printf "%-20s %-10s %-40s\n" "--------" "------" "----"
    
    for plugin in "${!GS_PLUGIN_REGISTRY[@]}"; do
        local status="${GS_PLUGIN_STATE[$plugin]:-unknown}"
        local manifest_file="${GS_PLUGIN_MANIFESTS[$plugin]}"
        local description
        description=$(jq -r '.description // "无描述"' "$manifest_file")
        
        # 应用过滤器
        case "$filter" in
            "enabled")
                [[ "$status" == "loaded" ]] || continue
                ;;
            "available")
                [[ "$status" != "loaded" ]] || continue
                ;;
        esac
        
        printf "%-20s %-10s %-40s\n" "$plugin" "$status" "$description"
    done
}

# 插件启用实现
gs_plugins_enable_cmd() {
    [[ $# -gt 0 ]] || gs_error "用法: gs-plugins-enable <plugin> [...]"
    
    local enabled_plugins=()
    local failed_plugins=()
    
    for plugin in "$@"; do
        if gs_plugin_enable "$plugin"; then
            enabled_plugins+=("$plugin")
        else
            failed_plugins+=("$plugin")
        fi
    done
    
    # 报告结果
    if [[ ${#enabled_plugins[@]} -gt 0 ]]; then
        echo "已启用插件: ${enabled_plugins[*]}"
    fi
    
    if [[ ${#failed_plugins[@]} -gt 0 ]]; then
        echo "启用失败: ${failed_plugins[*]}" >&2
        return 1
    fi
}
```

## 🚀 性能优化设计

### 6.1 启动性能优化

#### 懒加载策略
```bash
# 懒加载实现
lazy_loading_strategy() {
    # 阶段1: 核心系统初始化 (< 50ms)
    stage1_core_init() {
        load_essential_modules
        setup_basic_environment
        register_core_commands
    }
    
    # 阶段2: 按需插件加载 (< 100ms per plugin)
    stage2_plugin_loading() {
        # 只有在实际调用时才加载插件
        lazy_load_plugin_on_demand
    }
    
    # 阶段3: 缓存预热 (后台异步)
    stage3_cache_warming() {
        # 异步预热常用数据
        warm_cache_async &
    }
}

# 按需加载实现
lazy_load_command() {
    local command="$1"
    shift
    local args=("$@")
    
    # 查找命令对应的插件
    local plugin_info="${GS_COMMAND_REGISTRY[$command]}"
    if [[ -z "$plugin_info" ]]; then
        gs_error "未知命令: $command"
    fi
    
    local plugin_name="${plugin_info%%:*}"
    local function_name="${plugin_info#*:}"
    
    # 检查插件是否已加载
    if [[ "${GS_PLUGIN_STATE[$plugin_name]}" != "loaded" ]]; then
        gs_log "DEBUG" "按需加载插件: $plugin_name"
        gs_plugin_load "$plugin_name"
    fi
    
    # 执行命令
    "$function_name" "${args[@]}"
}
```

#### 并行初始化
```bash
# 并行初始化设计
parallel_initialization() {
    # 启动性能监控
    gs_perf_start_timer "bootstrap"
    
    # 并行执行独立的初始化任务
    {
        initialize_cache_system
        echo "cache_init_done" > "$GS_TEMP_DIR/init.cache"
    } &
    
    {
        initialize_configuration
        echo "config_init_done" > "$GS_TEMP_DIR/init.config"
    } &
    
    {
        discover_plugins
        echo "plugin_discovery_done" > "$GS_TEMP_DIR/init.plugins"
    } &
    
    # 等待关键任务完成
    wait_for_critical_init
    
    # 等待所有任务完成
    wait
    
    gs_perf_end_timer "bootstrap"
}

# 等待关键初始化完成
wait_for_critical_init() {
    local timeout=5  # 5秒超时
    local elapsed=0
    
    while [[ $elapsed -lt $timeout ]]; do
        if [[ -f "$GS_TEMP_DIR/init.config" ]]; then
            break
        fi
        sleep 0.1
        ((elapsed++))
    done
    
    if [[ ! -f "$GS_TEMP_DIR/init.config" ]]; then
        gs_error "配置初始化超时"
    fi
}
```

### 6.2 双级缓存系统

#### L1内存缓存
```bash
# L1缓存设计
declare -gA GS_CACHE_L1=()             # 内存缓存
declare -g GS_CACHE_L1_MAX_SIZE=1000   # 最大条目数
declare -g GS_CACHE_L1_TTL=3600        # 默认TTL(秒)

# L1缓存操作
gs_cache_l1_get() {
    local key="$1"
    local cached_data="${GS_CACHE_L1[$key]}"
    
    [[ -n "$cached_data" ]] || return 1
    
    local timestamp="${cached_data%%:*}"
    local value="${cached_data#*:}"
    local current_time=$(date +%s)
    
    if [[ $((current_time - timestamp)) -lt $GS_CACHE_L1_TTL ]]; then
        echo "$value"
        return 0
    else
        unset "GS_CACHE_L1[$key]"  # 过期清理
        return 1
    fi
}

gs_cache_l1_set() {
    local key="$1"
    local value="$2"
    local timestamp=$(date +%s)
    
    # 检查缓存大小限制
    if [[ ${#GS_CACHE_L1[@]} -ge $GS_CACHE_L1_MAX_SIZE ]]; then
        gs_cache_l1_evict
    fi
    
    GS_CACHE_L1[$key]="$timestamp:$value"
}

# L1缓存淘汰策略 (简单LRU)
gs_cache_l1_evict() {
    local evict_count=$((${#GS_CACHE_L1[@]} / 4))  # 淘汰25%
    local -a keys_to_evict=()
    local count=0
    
    for key in "${!GS_CACHE_L1[@]}"; do
        [[ $count -lt $evict_count ]] || break
        keys_to_evict+=("$key")
        ((count++))
    done
    
    for key in "${keys_to_evict[@]}"; do
        unset "GS_CACHE_L1[$key]"
    done
    
    gs_log "DEBUG" "L1缓存淘汰: 移除 $count 条目"
}
```

#### L2磁盘缓存
```bash
# L2缓存设计
declare -g GS_CACHE_L2_DIR="$HOME/.gs/cache"
declare -g GS_CACHE_L2_TTL=86400        # 24小时TTL

# L2缓存操作
gs_cache_l2_get() {
    local key="$1"
    local cache_file="$GS_CACHE_L2_DIR/$(echo -n "$key" | sha256sum | cut -d' ' -f1).cache"
    
    [[ -f "$cache_file" ]] || return 1
    
    # 检查文件是否过期
    local file_time
    file_time=$(stat -f%m "$cache_file" 2>/dev/null || stat -c%Y "$cache_file" 2>/dev/null)
    local current_time=$(date +%s)
    
    if [[ $((current_time - file_time)) -lt $GS_CACHE_L2_TTL ]]; then
        cat "$cache_file"
        return 0
    else
        rm -f "$cache_file"  # 过期删除
        return 1
    fi
}

gs_cache_l2_set() {
    local key="$1"
    local value="$2"
    local cache_file="$GS_CACHE_L2_DIR/$(echo -n "$key" | sha256sum | cut -d' ' -f1).cache"
    
    mkdir -p "$GS_CACHE_L2_DIR"
    echo "$value" > "$cache_file"
}

# 统一缓存接口
gs_cache_get() {
    local key="$1"
    local default_value="$2"
    
    # 先检查L1缓存
    local value
    if value=$(gs_cache_l1_get "$key"); then
        echo "$value"
        return 0
    fi
    
    # 再检查L2缓存
    if value=$(gs_cache_l2_get "$key"); then
        # 回填L1缓存
        gs_cache_l1_set "$key" "$value"
        echo "$value"
        return 0
    fi
    
    # 缓存未命中
    echo "$default_value"
    return 1
}

gs_cache_set() {
    local key="$1"
    local value="$2"
    
    # 同时更新L1和L2缓存
    gs_cache_l1_set "$key" "$value"
    
    # 异步更新L2缓存
    (gs_cache_l2_set "$key" "$value") &
}
```

这个架构设计保持了原失败重构的优秀设计理念，但去除了复杂性，更加实用和可维护。重点体现在简洁的技术栈、清晰的分层架构、灵活的插件系统，以及高效的性能优化策略。